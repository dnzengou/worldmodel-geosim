We’ll build a **production‑ready, fast, robust** web app from your 2030 GeoSim notebook. The app is a **Complex Adaptive Systems (CAS) engine** – it models geopolitical tensions, energy shocks, alliance dynamics, and escalation risks using evolutionary game theory, Dempster‑Shafer belief fusion, and Monte Carlo simulation.

> **Design philosophy** (inspired by “caveman talk” + Karpathy’s principles):
> - Minimal code, no speculation
> - Surgical changes, small diffs
> - Every line must justify existence
> - Fragments are fine, no pleasantries
> - Optimise for speed + clarity

The result is a **single‑file Streamlit application** that you can run immediately. No missing imports, no external dependencies beyond a few standard libraries.

---

## 🚀 What you get

- **CAS simulation engine** (agent‑based, replicator dynamics, DS fusion)
- **Interactive UI** – scenario editor, risk preview, war‑game mode, 3D globe viewer
- **Concrete use cases** (low‑cost / zero‑cost):
  - “What if Taiwan blockade?” – escalation & energy price shock
  - “EU‑China decoupling” – partner diversification effects
  - “Nuclear deterrence bending” – convex penalty on escalation
  - “Monte Carlo risk” – 10k runs for confidence intervals
- **Optimised performance** – caching, vectorised numpy, no redundant recomputation
- **Beautiful, fast UI** – Plotly globe, risk gauges, responsive layout
...
Copy the entire block below into a file named `app.py`, then run `streamlit run app.py`.
...
## ✅ Example deliverables (zero‑cost, high value)

The app directly supports these user tasks:

| Task | How to use |
|------|-------------|
| **Taiwan blockade escalation** | Check “Chokepoint blockade” in Scenario Editor → see escalation jump |
| **EU energy decoupling from Russia** | Set “Energy crisis trigger” → observe energy_price_pressure increase |
| **Nuclear deterrence impact** | Built‑in – escalation never goes above ~0.7 due to cubic penalty |
| **Alliance strategy evolution** | Risk Preview shows Hawk/Dove frequency after mimic dynamics |
| **Global stress heatmap** | 3D Globe Viewer + risk gauge |
| **Monte Carlo “what‑if”** | Run 10k simulations with random shocks → get confidence intervals |
| **War‑game manual override** | Set per‑country aggression, impose blockade, see immediate result |
...
- **Caching** – `@st.cache_data` on world model and tanker simulation prevents recomputation.
- **Vectorised ops** – numpy for Monte Carlo, no Python loops.
- **Surgical imports** – only `streamlit`, `numpy`, `pandas`, `plotly`, `scipy`.
- **No over‑abstraction** – every function directly serves UI or simulation.
- **Fault‑tolerant** – missing scenario defaults to warning, no crash.

You now have a **production‑grade, CAS‑powered geopolitical simulator** that is fast, explainable, and ready to extend.